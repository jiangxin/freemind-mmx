...
public class WSL_Main extends  ExportHook {
	...
	public void startupMapHook() {
	    ...
	    if(modelChecking()){
	    	...
	    	String configFile= curDir + "/plugins/WSL/resources/localSettings.conf";
...
//				catch (IOException e) {
//			    JOptionPane.showMessageDialog (null, e.toString(), "WSL error", JOptionPane.ERROR_MESSAGE);
//			}
	    }
...
//		try {
		    ...
		    InputStream xsltFile = getClass().getClassLoader().getResourceAsStream("wsl_resources/WSL_mm2php.xsl");
...
//		try {
			File mySQLFile = new File (curDir+"/plugins/WSL/resources/mySQLScript.sql");
			File currentMMFile = getCurrentFile();
	        // search xslt file and apply the transformation
		    InputStream xsltFile = getClass().getClassLoader().getResourceAsStream("wsl_resources/WSL_mm2sql.xsl");
		    ...
		    while (m1.find()) {
			    ...
			    InputStream xsltFileWikiMarkup = getClass().getClassLoader().getResourceAsStream("wsl_resources/word2wiki/createWikiMarkup.xsl");
		    ...
		    while (m2.find()) {
			    // Content text from WordProcessingML (XML format generated by Microsoft Word 2003 and 2007) http://sourceforge.net/projects/word2wiki/
			    InputStream xsltFileWikiMarkup = getClass().getClassLoader().getResourceAsStream("wsl_resources/word2wiki/createTemplateWikiMarkup.xsl");
		    	//Entire pattern found
...
//
	...
	private void installMediaWikiExtensions() throws IOException {
		File extensionsRoot = new File (curDir+"/plugins/WSL/resources/extensions");
	    ...
	    for (int i = 0; i < sourceExtensions.length; i++) {
	    	if (sourceExtensions[i].isDirectory() && !extensions.toLowerCase().contains(sourceExtensions[i].getName().toLowerCase())) {
	    		File dir = new File(localSettings.getParent()+"/extensions/"+sourceExtensions[i].getName());
	...
	private void installMediaWikiSkins() throws IOException {
		File extensionsRoot = new File (curDir+"/plugins/WSL/resources/skins");
	    File[] sourceExtensions = extensionsRoot.listFiles();
	    for (int i = 0; i < sourceExtensions.length; i++) {
	    	if (sourceExtensions[i].isDirectory()) {
	    		wsl_util.copyFolder(sourceExtensions[i], new File(localSettings.getParent()+"/skins/"+sourceExtensions[i].getName()));
	    	}
	    	else if (sourceExtensions[i].isFile()){
	    		wsl_util.copyFile(sourceExtensions[i], new File(localSettings.getParent()+"/skins/"+sourceExtensions[i].getName()));
	    	}
	     }
	}
