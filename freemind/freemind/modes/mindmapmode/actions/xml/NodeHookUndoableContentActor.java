...
public class NodeHookUndoableContentActor implements ActorXml {
	...
	public void performAction(MindMapNode focussed, String hookName,
			String attributeName, String content) {
		String oldContent = findHook(focussed, hookName).getContent(
				attributeName);
		if (oldContent.equals(content)) {
			return;
		}
		XmlAction doAction = createHookContentNodeAction(focussed, hookName,
				attributeName, content);
		XmlAction undoAction = createHookContentNodeAction(focussed, hookName,
				attributeName, oldContent);
	...
	private void invoke(MindMapNode node, String hookName,
			String attributeName, String content) {
	...
	private StatefulNodeHook findHook(MindMapNode node, String hookName) {
		for (Iterator j = node.getActivatedHooks().iterator(); j.hasNext();) {
			...
			if (permhook instanceof StatefulNodeHook
					&& permhook.getName().equals(hookName)) {
	...
	public HookContentNodeAction createHookContentNodeAction(
			MindMapNode focussed, String hookName, String attributeName,
			String content) {
	...
	public void act(XmlAction action) {
		if (action instanceof HookContentNodeAction) {
			...
			MindMapNode selected = getController().getNodeFromID(
					hookContentNodeAction.getNode());
			invoke(selected, hookContentNodeAction.getHookName(),
					hookContentNodeAction.getAttributeName(),
					hookContentNodeAction.getContent());
		}
	}
