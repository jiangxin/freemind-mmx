...
//				logger.info("onReceiveFocuse for node " + node.toString());
				...
				if (note != null) {
					getHtmlEditorPanel()
							.setCurrentDocumentContent(note);
		...
		/**
		 * Indicates, whether or not the main panel has to be refreshed with 
		 * new content. The typical content will be empty, so this state is saved here.
		 */
