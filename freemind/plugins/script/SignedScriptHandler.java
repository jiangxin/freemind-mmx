 ...
 */
/*$Id: SignedScriptHandler.java,v 1.1.2.1 2008-04-17 19:32:30 christianfoltin Exp $*/
...
public class SignedScriptHandler {
	...
	private static final String FREEMIND_SCRIPT_KEY_NAME = "FreeMindScriptKey";
	...
	public String signScript(String pScript, TextTranslator pTranslator, FreeMindMain pFrame) {
		ScriptContents content = getScriptContent(pScript);
...
//			System.out.println("Signed: "  +content);
	...
	public boolean isScriptSigned(String pScript) {
		ScriptContents content = getScriptContent(pScript);
		if (content.mSignature != null) {
			try {
				Signature instanceVerify = Signature.getInstance("SHA1withDSA");
				/** This is the FreeMind public key.
				 * keytool -v -rfc -exportcert -alias freemindscriptkey
				 *  */
	...
	public ScriptContents getScriptContent(String pScript) {
		ScriptContents content = new ScriptContents();
		...
		if (indexOfSignaturePrefix > 0 && pScript.length() > indexOfSignature) {
			content.mSignature = pScript.substring(indexOfSignature);
			content.mScript = pScript.substring(0, indexOfSignaturePrefix);
			content.mKeyName = null;
		} else {
			content.mSignature = null;
			content.mScript = pScript;
			content.mKeyName = null;
		}
		return content;
	}
