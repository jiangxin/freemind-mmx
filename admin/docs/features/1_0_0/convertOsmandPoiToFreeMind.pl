...
foreach my $row (@$res) {
  ...
  print "<hook NAME=\"plugins/map/MapNodePositionHolder.properties\">\n<Parameters XML_STORAGE_MAP_LAT=\"$lat\" XML_STORAGE_MAP_LON=\"$lon\" XML_STORAGE_POS_LAT=\"$lat\" XML_STORAGE_POS_LON=\"$lon\" XML_STORAGE_TILE_SOURCE=\"org.openstreetmap.gui.jmapviewer.tilesources.OsmTileSource\$Mapnik\" XML_STORAGE_ZOOM=\"20\"/>\n</hook>\n";
  print "</node>\n";
